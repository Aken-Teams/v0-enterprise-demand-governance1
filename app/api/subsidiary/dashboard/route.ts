...
export async function GET(request: NextRequest) {
  try {
    ...
    // Restricted users (with DemandAccess whitelist) cannot access the dashboard
    if (accessCount > 0) {
      return NextResponse.json({ error: "您的帳號無權查看總覽" }, { status: 403 })
    }
    ...
    // Average dev days: non-rejected demands with DEVELOPING phase planned dates
