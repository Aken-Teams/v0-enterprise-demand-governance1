...
interface Props {
  ...
  dcId?: string
  /** revise 時帶入上一版內容 */
  initial?: { title: string; summary: string; checklistMd: string; affectsSp: boolean; spNote: string }
  onComplete: () => void
}

export function DesignChangeEditorDialog({
  open, onOpenChange, demandId, demandNumber, phaseLabel, token, mode, dcId, initial, onComplete,
}: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (affectsSp && !spNote.trim()) { setError("請說明 SP 影響內容"); return }
    if (!token) return
    ...
    try {
      ...
      if (affectsSp) fd.append("spNote", spNote.trim())
    ...
    <Dialog open={open} onOpenChange={(o) => { if (!loading) onOpenChange(o) }}>
      <DialogContent className="max-w-[calc(100%-1rem)] sm:max-w-2xl max-h-[90vh] flex flex-col p-4 sm:p-6">
          ...
          <div className="rounded-md border border-amber-200 bg-amber-50/60 p-2.5 sm:p-3 space-y-2">
            <div className="flex items-start gap-2">
              ...
              <Label htmlFor="dc-affects-sp" className="text-xs sm:text-sm font-medium cursor-pointer leading-snug">
                此變更影響 SP（勾選後將加入<strong>董事會</strong>審核）
              </Label>
            </div>
            {affectsSp && (
              <Textarea value={spNote} onChange={(e) => { setSpNote(e.target.value); setError("") }}
                placeholder="說明 SP 影響（例如：預估增加 5 SP，因新增檔案格式解析）..." rows={2} disabled={loading} className="text-xs sm:text-sm" />
            )}
          </div>
        ...
        <DialogFooter className="gap-2 sm:gap-2">
          ...
          <Button size="sm" className="h-8 text-xs sm:text-sm bg-indigo-600 hover:bg-indigo-700 text-white" onClick={handleSubmit} disabled={loading}>
