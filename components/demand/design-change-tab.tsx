...
import {
  ...
  History, CircleDollarSign, FileText, ListChecks, Maximize2, Trash2, FileIcon, Eye,
} from "lucide-react"
import { DESIGN_CHANGE_STATUS_MAP, CHECKLIST_MARK_MAP } from "@/lib/constants/demand"
...
interface DcDocument { id: string; fileName: string; fileUrl: string | null; fileSize: number | null; docGroup: string | null; fileVersion: number; uploadedBy: { name: string }; createdAt: string }
interface Revision {
...
export function DesignChangeTab({ demandId, demandNumber, phaseLabel, token, currentUserId, canManage, watermarkBg, onPreviewDoc }: Props) {
  ...
  const [draft, setDraft] = useState<Record<string, { items: Record<string, { mark: Mark; comment: string }>; comment: string; files: File[] }>>({})
  ...
  const dd = (dcId: string) => draft[dcId] ?? { items: {}, comment: "", files: [] }
  const setItemMark = (dcId: string, itemId: string, mark: Mark) => setDraft((d) => {
    const cur = d[dcId] ?? { items: {}, comment: "", files: [] }
  ...
  const setOverall = (dcId: string, comment: string) => setDraft((d) => ({ ...d, [dcId]: { ...(d[dcId] ?? { items: {}, comment: "", files: [] }), comment } }))
  const addFiles = (dcId: string, list: FileList) => {
    const arr = Array.from(list).filter((f) => f.size <= MAX_FILE_SIZE)
    setDraft((d) => ({ ...d, [dcId]: { ...(d[dcId] ?? { items: {}, comment: "", files: [] }), files: [...(d[dcId]?.files ?? []), ...arr] } }))
  }
  const removeFile = (dcId: string, idx: number) => setDraft((d) => ({ ...d, [dcId]: { ...(d[dcId] ?? { items: {}, comment: "", files: [] }), files: (d[dcId]?.files ?? []).filter((_, i) => i !== idx) } }))
  ...
  const submitReview = async (dc: DesignChange, rev: Revision, decision: "APPROVED" | "REJECTED") => {
    ...
    cur.files.forEach((f) => fd.append("files", f))
    setSubmitting(dc.id)
    try {
      ...
      if (res.ok) { setDraft((d) => ({ ...d, [dc.id]: { items: {}, comment: "", files: [] } })); await load() }
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
              ...
              {isOpen && (
                <div className="border-t">
                  ...
                  <div className="grid gap-3 sm:gap-4 lg:grid-cols-5 p-3 sm:p-4">
                    {/* LEFT: 預覽卡 */}
                    <Card className="lg:col-span-3 min-h-[320px] lg:min-h-[460px]">
                      ...
                      {!selection ? (
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center gap-2 text-sm text-muted-foreground"><ListChecks className="h-4 w-4" />檢查清單 · 共 {rev.items.length} 項{iReview ? "（請逐條確認或標記問題）" : ""}</div>
                            ...
                            return (
                              <div key={it.id} className="rounded-md border p-3">
                                <div className="flex items-start gap-2">
                                  {iReview ? (
                                    <Checkbox className="mt-0.5" checked={myMark === "CONFIRMED"} onCheckedChange={(v) => setItemMark(dc.id, it.id, v ? "CONFIRMED" : "PENDING")} />
                                  ) : (
                                    <span className={cn("text-base leading-none mt-0.5", it.devChecked ? "text-emerald-600" : "text-slate-300")}>{it.devChecked ? "☑" : "☐"}</span>
                                  )}
                                  <div className="flex-1 min-w-0">
                                    <p className="text-[15px] font-medium">{idx + 1}. {it.text}</p>
                                    ...
                                    {iReview && (
                                      <div className="mt-2 space-y-2">
                                        <div className="flex items-center gap-1.5">
                                          {(["WARN", "CROSS"] as Mark[]).map((m) => {
                                            const mk = CHECKLIST_MARK_MAP[m]; const active = myMark === m
                                            return (
                                              <button key={m} onClick={() => setItemMark(dc.id, it.id, active ? "PENDING" : m)}
                                                className={cn("text-sm px-2 py-1 rounded border flex items-center gap-1", active ? mk.color + " bg-muted border-current" : "text-muted-foreground hover:bg-muted")}>
                                                <span className="font-bold">{mk.icon}</span>{mk.label}
                                              </button>
                                            )
                                          })}
                                        </div>
                                        {(myMark === "CROSS" || myMark === "WARN") && (
                                    ...
                                    {it.feedback.length > 0 && (
                                      <div className="mt-2 pt-2 border-t space-y-1">
                                        {it.feedback.map((fb) => {
                                          const mk = CHECKLIST_MARK_MAP[fb.mark]
                                          return (
                                            <div key={fb.reviewerId} className="text-sm flex items-start gap-1.5">
                                              <span className={cn("font-bold", mk.color)}>{mk.icon}</span>
                    ...
                    <Card className="lg:col-span-2 flex flex-col">
                      <div className="p-3 space-y-3 overflow-y-auto lg:max-h-[460px]">
                        {/* 檔案 */}
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <p className="text-sm font-semibold text-muted-foreground flex items-center gap-1.5"><FileText className="h-4 w-4" />檔案</p>
                            {canManage && (
                              <button className="text-xs text-indigo-600 hover:underline" onClick={() => { setUploadTarget(dc.id); fileInputRef.current?.click() }}>+ 上傳</button>
                            )}
                          </div>
                          {rev.documents.length === 0 ? <p className="text-xs text-muted-foreground">無附件</p> : (
                      ...
                      {iReview && (
                        <div className="border-t p-3 space-y-2 mt-auto">
                          ...
                          <div className="flex items-center justify-between gap-2">
                            <button className="text-[11px] text-muted-foreground hover:text-foreground flex items-center gap-1" onClick={() => { setUploadTarget("review:" + dc.id); fileInputRef.current?.click() }}>
                            ...
                            <div className="flex gap-2">
                              ...
                              <Button size="sm" className="h-8 text-xs bg-emerald-600 hover:bg-emerald-700 text-white" disabled={submitting === dc.id} onClick={() => submitReview(dc, rev, "APPROVED")}>
                  ...
                  <div className="px-3 sm:px-4 py-3 border-t space-y-3">
                    {rev.reviews.length > 0 && (
                      <div>
                        <p className="text-[11px] font-semibold text-muted-foreground mb-1.5">審核裁決</p>
                        <div className="space-y-1">
                          {rev.reviews.map((r) => (
                            <div key={r.reviewerId} className="flex items-center gap-2 text-[11px] flex-wrap">
      ...
      <input ref={fileInputRef} type="file" multiple className="hidden"
        ...
        onChange={async (e) => {
          const files = e.target.files; const target = uploadTarget
          e.target.value = ""; setUploadTarget(null)
          if (!files || !files.length || !target) return
          if (target.startsWith("review:")) { addFiles(target.slice(7), files); return }
          // 上傳到該版本的檔案
          if (!token) return
    ...
    <div className="relative">
      <div className="flex items-center justify-between mb-2">
        ...
        {onFullScreen && (isText || isImg || isPdf) && (
          <button className="text-[11px] text-muted-foreground hover:text-foreground flex items-center gap-1" onClick={() => onFullScreen(toPreview())}><Maximize2 className="h-3 w-3" />全螢幕</button>
        )}
      </div>
      <div className="relative">
        {body}
